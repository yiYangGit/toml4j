  ...
  public void write(Object value, WriterContext context) {
    ...
    for (Map.Entry<?, ?> entry : from.entrySet()) {
      ...
      if (valueWriter.isPrimitiveType()) {
        context.indent();
        context.write(StringValueReaderWriter.quoteKey(key.toString())).write(" = ");
    ...
    for (Object key : from.keySet()) {
      ...
      if (valueWriter == this || valueWriter == ObjectValueWriter.OBJECT_VALUE_WRITER || valueWriter == TABLE_ARRAY_VALUE_WRITER) {
        valueWriter.write(fromValue, context.pushTable(StringValueReaderWriter.quoteKey(key.toString())));
      }
    }
  }
