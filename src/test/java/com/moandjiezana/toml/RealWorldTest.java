...
public class RealWorldTest {
  ...
  public void should_parse_hard_example() throws Exception {
    ...
    public void should_parse_current_version_example() throws Exception {
        Toml toml2 = new Toml();
        File file = new File(getClass().getResource("example-v0.4.0.toml").getFile());
        InputStreamReader reader = new InputStreamReader(new FileInputStream(file), "UTF8");
        TomlContainer tomlContainer = toml2.readerToTomlContainer(reader);
        ...
        for (int i = 0; i < 100; i++) {
            ...
            TomlContainer tomlContainer1 = toml2.readerToTomlContainer(new StringReader(s));
            tomlWriter.write(tomlContainer1, target2);
            s = target2.toString();
        }
        ...
        assertEquals("\u00E9\n\u111111", toml.getString("string.basic.basic2"));
        assertEquals("", toml.getString("string.basic.basic3"));
        ...
        assertEquals("The\\\u1111\b\n\f\r\t quick brown fox jumps over the lazy dog.", toml.getString("string.multiline.continued.key4"));
