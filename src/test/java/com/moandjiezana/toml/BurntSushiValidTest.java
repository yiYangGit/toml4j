...
public class BurntSushiValidTest {
    ...
    public void key_newline() throws Exception {
        String name = "key-newline";
        InputStream inputToml = getClass().getResourceAsStream("burntsushi/valid/" + name + ".toml");
        Toml read = new Toml().read(inputToml);
        assertEquals(read.getLong("a"), new Long(1));
    }
  ...
  private void run(String testName) {
    ...
    JsonElement actual = TEST_GSON.toJsonTree(toml.toMap());
